...
st.title("Bases teóricas para obtenção da matriz de rigidez por método de propagação de ondas")

st.header("Início")

st.write("As bases teóricas envolvidas na obtenção da matriz de rigidez depende do tipo de material analisado, tendo em vista que ele pode apresentas comportamento elástico distinto em diferentes direções. O capítulo relacionado à atenuação de ondas acústicas em sólidos discute com maior ênfase os diferentes fatores que podem afetar a propagação de ondas acústicas e modificar a sua velocidade ao passar pelo material. Dessa forma, torna-se fundamental entender as características do tipo de material ensaiado visando adequar o tamanho, a forma e o tipo de transdutores utilizados. Diversos estudos foram realizados para avaliação do método de propagação de ondas na avaliação das propriedades de rigidez de um material. É sabido que existe uma correlação entre os coeficientes de rigidez em determinadas direções e as propriedades de resistência e rigidez dos materiais. Prova disso, são os ensaios realizados em amostras de madeira em forma de barra, onde são propagados pulsos ultrassônicos na direção longitudinal e, com o valor da densidade $\\rho$ calcula-se o coeficiente de rigidez definido por")

st.latex(r"C_{LL}=\rho V_{LL}^{2}")

st.write("Todavia esse método é restrito somente a algumas condições tendo em vista que o comportamento elástico altamente anisotrópico da madeira dificulta a relação do coeficiente de rigidez em outras direções.")

st.write("Sabe-se que a rigidez é uma propriedade relacionada à dificuldade de um corpo deformar quando submetido a um carregamento, seja de compressão, tração, cisalhamento ou flexão. A lei de Hooke generalizada, define que um estado tridimensional de tensões em formato cúbico de material isotrópico, quando submetido a esforços de tensão normal e cisalhamento em todas as faces, tem as seguintes deformações específicas em diferentes direções e planos")
...
st.write("enquanto que para materiais ortotrópicos")
...
st.write("Na forma matricial, podemos escrever o mesmo conjunto de equações como")
...
st.write("Note que, a partir do sistema matricial obtido, podemos simplificar sua escrita como")

st.latex(r"\bm{\epsilon}=\bm{S}\bm{\sigma}")

st.markdown("onde $\\bm{\\epsilon}$ é o vetor de deformações específicas, $\\bm{S}$ é a matriz *compliance* e $\\bm{\\sigma}$ é o vetor de tensões. Isolando o vetor de tensões, chega-se que")

st.latex(r"\bm{\sigma}=\bm{C}\bm{\epsilon}")

st.markdown("consequentemente conclui-se que $\\bm{S}=\\bm{C}^{-1}$, onde $\\bm{C}$ é a matriz de rigidez.")

st.header("Materiais isotrópicos")

st.subheader("Matriz de rigidez")

st.markdown("Quando lidamos com a matriz de rigidez para materiais isotrópicos, temos que levar em conta as simetrias envolvidas no material e como suas propriedades variam em diferentes direções. A matriz de rigidez nesse caso é simplificada e depende de somente 3 coeficientes de rigidez do material ($C_{11}$, $C_{12}$ e $C_{44}$), como é mostrado a seguir")
...
st.write("em termos das velocidade de propagação das ondas longitudinais e transversais")
...
st.subheader("Matriz de flexibilidade")
...
st.markdown("se expandirmos esses coeficientes, temos que tanto o módulo de elasticidade longitudinal quanto o transversal e o coeficiente de Poisson independem da direção assumida. Dessa forma, em termos das propriedades elásticas do material, ele só depende de $\\nu$, $E$ e $G$ na matriz *compliance* $\\bm{S}$")
...
st.write("Nota-se que a matriz possui 5 coeficientes de rigidez independentes ($C_{11}$, $C_{33}$, $C_{44}$, $C_{12}$ e $C_{13}$).")

st.subheader("Plano 13 de isotropia")
...
st.markdown("**Matriz de flexibilidade**")
...
st.subheader("Plano 23 de isotropia")
...
st.header("Materiais ortotrópicos")

st.subheader("Matriz de rigidez")

st.write("Para materiais ortotrópicos, observa-se maior complexidade em sua estrutura em relação aos dois tipos de materiais anteriores. Neste caso, há dependência de 9 coeficientes de rigidez.")
...
st.write("em termos das velocidades de propagação das ondas longitudinais, transversais, quase longitudinais e quase transversais, temos")
...
st.subheader("Propagação de ondas no plano 13")
...
st.subheader("Propagação de ondas no plano 23")
