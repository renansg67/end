...
st.title("Atenuação de ondas acústicas em sólidos")

st.header("Início")

st.write("São vários os mecanismos de atenuação de ondas acústicas em sólidos. Neste capítulo será visto inicialmente um abordagem mais generalizada para vários tipos de materiais e os mecanismos de atenuação envolvidos com base em suas características e particularidades. Serão tratados os principais fatores relacionados à atenuação de ondas acústicas em sólidos, em materiais isotrópicos, fontes dispersoras de ondas acústicas, do que o impacto delas depende e materiais com pouca rigidez. Em seguida, numa abordagem focada na madeira, serão apresentadas as bases teóricas para o cálculo do coeficiente de atenuação e, por fim, os fatores que afetam a medida da atenuação, tratando da geometria da amostra e das características do material.")

st.subheader("Fatores principais relacionados à atenuação")

st.write("As ondas sonoras diminuem de intensidade quanto maior a distância em relação à fonte. Deve-se atentar que apesar de nos ensaios envolvendo amostras de pequeno porte a reflexão do feixe ultrassônico aproximá-lo da fonte emissora, a atenuação ocorre da mesma forma, estando associada à distância percorrida. A perda de intensidade geralmente se dá devido ao espalhamento geométrico, dispersão e absorção.")

st.subheader("Atenuação em materiais isotrópicos")

st.write("Em materiais isotrópicos, a atenuação depende das características da fonte irradiante. Ou seja, dependendo do padrão de direcionalidade do feixe ultrassônico do transdutor isso pode alterar o grau de atenuação do feixe. Feixes menos divergentes viajam mais distante no material.")

st.subheader("Materiais com fontes dispersoras")

st.write("Em materiais com dispersores como: fronteiras de grãos, inclusões segregadas e poros de gás, a atenuação ocorre devido a: Reflexão do feixe ultrassônico ao encontrar descontinuidades; redirecionamento parcial do feixe de onda plana coerente ao passar pelo material e dispersão da energia em várias direções.")

st.subheader("Impacto das fontes dispersoras")

st.write("O impacto das fontes de dispersão depende de seu tamanho em relação ao comprimento de onda do feixe ultrassônico. Dispersores menores que o comprimento de onda colaboram menos com a atenuação do sinal. Em dispersores com dimensões próximas do comprimento de onda do feixe emitido pelo transdutor o espalhamento torna-se problemático, havendo maior atenuação do sinal. Para corrigir isso, recomenda-se utilizar fontes com maior comprimento de onda, porém, como consequência há perda de sensibilidade à descontinuidades e de resolução.")

st.subheader("Atenuação em materiais com grãos colunares")

st.write("Alguns materiais com grãos colunares como: compósitos laminados e aços inoxidáveis apresentam comportamento elástico altamente anisotrópico. Neles a atenuação se dá devido às distorções do feixe de onda incidente e mudança de trajetória do feixe decorrente dos caminhos preferenciais existentes, podendo afetar a análise de sinais.")

st.subheader("Atenuação em materiais pouco rígidos")

st.write("Materiais pouco rígidos como: chumbo, plástico e materiais de acoplamento contribuem para maior atenuação do sinal. Neste contexto, a atenuação ocorre devido à conversão de parte da energia mecânica (acústica) em térmica e, devido ao fato da energia na forma de calor ser irrecuperável. Em materiais com essas características, recomenda-se ensaiar amostras com pequena espessura visando minimizar a absorção, principal fenômeno envolvido nesses materiais.")

st.subheader("Atenuação na madeira")

st.write("Em materiais como a madeira, a base teórica da atenuação de ondas acústicas depende do tensor de Christoffel, a partir das equações abaixo")
...
st.markdown("onde o autovalor $\\Lambda^{*}$ é dado por")
...
st.markdown("o mesmo depende do coeficiente de atenuação $\\alpha_{ij}$ característico do material. A técnica de medição da atenuação consiste em calcular o mesmo a partir da equação")
